...
HRESULT KinectSensor::processDepth()
{
	...
	//constrictions for the depthImage
	int x1 = getDepthWidth() / 6;
	int x2 = getDepthWidth() - x1; 
	int	y1 = getDepthHeight() / 4;
	int y2 = getDepthHeight() - (y1 + 17);
...
//#pragma omp parallel for
	...
	for (unsigned int j = 0; j < getDepthHeight(); j++) {
		for (unsigned int i = 0; i < getDepthWidth(); i++) {
			...
			/*const USHORT& pre_d = pBuffer[srcIdx-1].depth;
			const USHORT& post_d = pBuffer[srcIdx+1].depth;
			const USHORT dif1 = sqrt(pow((pre_d-d),2));
			const USHORT dif2 = sqrt(pow((post_d-d),2));*/

			
			//constraining sensed depth to a small sector of the image
			if (/*(j < y1) ||*/ (i < x1) || (j > y2) || (i > x2) || d < 100 || d > 1400)
			{
				m_depthD16[desIdx] = -100000;
				test[srcIdx] = -100000;
			}
			else
			{
	...
	for (int yi = 0; yi < (int)getColorHeight(); ++yi) {
		...
		for (LONG x = 0; x < (int)getColorWidth(); ++x) {
			...
			if ( colorInDepthX >= 0 && colorInDepthX < (int)getColorWidth() && colorInDepthY >= 0 && colorInDepthY < (int)getColorHeight() ) {
				...
				tmp |= 0xFF000033; // Flag for is valid
